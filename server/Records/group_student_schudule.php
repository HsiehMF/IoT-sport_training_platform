...
<?php 
    ...
    echo "<a class='collection-item active'>$schedule_name</a>";
	...
	while ($i <= count($Line_num)) {
		...
		for($i=2;$i<count($item_num)-1;$i++){
			...
			echo "<td><a class='waves-effect waves-light btn' href='http://140.127.218.198:8080/webapp/Records/lookdata_group.php?schedule_num=$schedule_num&schedule_date=$schedule_date&list_num=$j&item_name=$row3[item_name]&item_time=$row3[item_time]'>點擊查看</a></td>";
			echo "</tr>";
			$j++;
		}
		echo "</table>";

...
while ($row = mysqli_fetch_array($result)) {
	...
	if ($flag>0){
		$sql2="UPDATE `Self_evaluation` SET `grades`=\" $grades \" WHERE `schedule_num`= $schedule_num and `UID`= $UID and `Date` like \"%$curDay%\"";
		echo "sql2=".$sql2."<br>";
		...
		$sql2="INSERT INTO `Self_evaluation`(`UID`, `schedule_num`, `Date`, `grades`) VALUES ($UID,$schedule_num,\"$curDay\", \"  $grades  \" )";
		$result1 = mysqli_query($con, $sql2);
		echo "sql2=".$sql2."<br>";
